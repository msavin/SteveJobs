...
`Jobs.run` (async) allows you to schedule a job to run. You call it just like you would call a method, by specifying the job namea and its arguments. At the end, you can pass in a special configuration object. Otherwise, it will be scheduled to run as soon as possible.
...
`Jobs.find` (async) allows you to find a single pending job by its arguments, and run logic against it. You can pass in a callback in the end to check for the document, and you can run the same operations with-in the function scope as you would with `Jobs.register`, as well as `this.run` as a shortcut to `Jobs.run`.

```javascript
await Jobs.find("sendReminder", "jony@apple.com", "The future is here!", async function (jobDoc) {
	if (jobDoc) {
		await this.reschedule({
			in: {
				minutes: 5
...
`Jobs.execute` (async) allows you to run a job ahead of its due date. It can only work on jobs that have not been resolved. 
...
`Jobs.reschedule` (async) allows you to reschedule a job. It can only work on jobs that have not been resolved. 
...
`Jobs.replicate` (async) allows you to replicate a job.
...
`Jobs.stop` (async) allows you stop all the queues. If you call the function with no arguments, it will stop all the queues. If you pass in a String, it will stop a queue with that name. If you pass in an Array, it will loop over the items in it, and treat them like a string.
...
`Jobs.get` (async) allows you to get a job document by its document id.
...
`Jobs.cancel` (async) allows you to cancel a job if it has not run already.
...
`Jobs.clear` (async) allows you to clear all or some of the jobs in your database. It supports `state` for selecting a job state, which can be `pending`, `success`, or `failure`, or `*"`to select all of them.
...
`Jobs.remove` (async) allows you to remove a job from the collection.
